...
The source of the stock prices is from https://finance.yahoo.com/quote/BAESY where I downloaded all of the stock prices for BAE Systems (Sept 3 2014 - Sept 3, 2019). The variable to predict is the Adjusted Close price.
...
"""I have exported the BAE Systems stock price data to my personal Github and will be downloading it locally."""

# Download the data and store it
# BAE Systems stock, 5 years
...
"""To make this more robust to noise and to leverage past values, we will not use the date to predict the stock prices, but we will use batches of 90 prices where each training example, we leverage 90 prices from the past to predict the price on the 91st day. We will thus have N−90 training examples with N being the number of days we've downloaded for the stock data. We will thus create a 2D matrix of training samples with each row being the prices 90 consecutive days in the past with the price to predict being the current day."""
